...
execute unless predicate mcpaint:painting_mode run data modify storage mcpaint:calc join_studio.player_willing set value 1b

execute unless predicate mcpaint:painting_mode as @e[type=interaction,tag=mcpaint.studio.interaction,distance=..16] if data entity @s interaction at @s run function mcpaint:studio/interaction_get_right_clicked
